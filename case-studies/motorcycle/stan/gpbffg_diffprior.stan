...
model {
  ...
  target += normal_lpdf(beta_f | 0, beta_f_s); // non-centered
  target += normal_lpdf(beta_g | 0, beta_g_s); // non-centered
  target += prior_alpha * lognormal_lpdf(lengthscale_f | 0, lscale_f_s);
  target += prior_alpha * lognormal_lpdf(lengthscale_g | 0, lscale_g_s);
...
generated quantities {
  ...
  ratio_f = sigma_f^2 / lengthscale_f;
  ratio_g = sigma_g^2 / lengthscale_g;

  lprior = prior_alpha * (
			     normal_lpdf(intercept_f | 0, intercept_f_s) +
			     normal_lpdf(intercept_g | 0, intercept_g_s) +
			     lognormal_lpdf(lengthscale_f | 0, lscale_f_s) +
			     lognormal_lpdf(lengthscale_g | 0, lscale_g_s) +
